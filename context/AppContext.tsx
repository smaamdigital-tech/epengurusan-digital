...
import { User, Permissions, Announcement, Program, SiteConfig, PermissionKey } from '../types';
...
interface AppContextType {
  ...
  checkPermission: (module: PermissionKey, action: 'view' | 'edit' | 'delete' | 'save' | 'download') => boolean;
  activeTab: string;
...
const defaultActionPerms = { view: true, edit: true, delete: true, save: true, download: true };

const defaultPermissions: Permissions = {
  dashboard: { ...defaultActionPerms },
  pengguna: { ...defaultActionPerms },
  jawatankuasa: { ...defaultActionPerms },
  tugasFungsi: { ...defaultActionPerms },
  program: { ...defaultActionPerms },
  dokumentasi: { ...defaultActionPerms },
  laporan: { ...defaultActionPerms },
  tetapan: { ...defaultActionPerms, view: false, edit: false, save: false, delete: false }, // Default restricted
};
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const savedPermissions = localStorage.getItem('smaam_permissions_v2'); // Changed key to force refresh on new version
    if (savedPermissions) {
  ...
  const updatePermissions = (newPermissions: Permissions) => {
    ...
    localStorage.setItem('smaam_permissions_v2', JSON.stringify(newPermissions));
  };

  const checkPermission = (module: PermissionKey, action: 'view' | 'edit' | 'delete' | 'save' | 'download'): boolean => {
    // System Admin has full access regardless of permission settings
    if (user?.role === 'adminsistem') return true;
    
    // Standard Admin follows the configured permissions
    if (user?.role === 'admin') {
      return permissions[module][action];
    }
    
    // Guest/No User - Read only for View, Block others
    if (!user) {
        if (action === 'view') return true; // Or false if you want to block guests completely
        return false;
    }
  ...
  const saveToCloud = async () => {
    ...
    try {
      ...
      if (contentType && contentType.indexOf("text/html") !== -1) {
          throw new Error("DEPLOYMENT ERROR: Sila set 'Execute as: Me' dan 'Access: Anyone' pada skrip.");
      }
      ...
      console.error(error);
      const msg = error.message?.includes("DEPLOYMENT ERROR") ? error.message : "❌ Gagal menyambung. Semak Console.";
      showToast(msg);
      if (error.message?.includes("DEPLOYMENT ERROR")) {
          alert("AMARAN KESELAMATAN GOOGLE DITEMUI:\nSila semak 'Panduan Elak Warning' di Tetapan Admin.");
      }
    } finally {
      setIsSyncing(false);
    }
    ...
    try {
       ...
       if (result.status === 'success' && result.data) {
          ...
          if(d.permissions) {
             ...
             localStorage.setItem('smaam_permissions_v2', JSON.stringify(d.permissions));
          }
          if(d.siteConfig) {
       ...
       console.error(error);
       const msg = error.message?.includes("DEPLOYMENT ERROR") ? error.message : "❌ Gagal memuat turun data.";
       showToast(msg);
       if (error.message?.includes("DEPLOYMENT ERROR")) {
          alert("AMARAN KESELAMATAN GOOGLE DITEMUI:\nSila semak 'Panduan Elak Warning' di Tetapan Admin.");
      }
  ...
  return (
    <AppContext.Provider value={{
      user, login, logout,
      permissions, updatePermissions, checkPermission,
