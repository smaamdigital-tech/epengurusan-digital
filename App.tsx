...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    if (activeTab.includes(' - ')) {
      ...
      // Special handler for Kurikulum > Peperiksaan
      if (parent === 'Kurikulum' && child === 'Peperiksaan') {
        return <KurikulumPeperiksaan />;
      }

      // Special handler for HEM > Kehadiran Murid
      if (parent === 'Hal Ehwal Murid' && child === 'Kehadiran Murid') {
        return <HemKehadiran />;
      }
  ...
  return (
    <div className="flex min-h-screen bg-[#0B132B] text-[#F8F9FA] font-sans overflow-x-hidden">
      ...
      <div className="flex-1 flex flex-col relative w-full md:ml-64 transition-all duration-300">
        ...
        <main className="flex-1 overflow-y-auto bg-[#0B132B] w-full flex flex-col">
          <div className="flex-1">
            {renderContent()}
          </div>
          
          {/* Footer */}
          <footer className="w-full bg-[#1C2541] border-t border-[#C9B458] py-4 mt-auto text-center shadow-inner relative z-10">
            <div className="text-white text-[14px] font-medium font-inter tracking-wide">
               Hak Milik ePengurusan Digital SMAAM @ 2026
            </div>
            <div className="text-gray-400 text-[14px] font-light mt-1">
