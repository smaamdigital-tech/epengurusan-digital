...
import { menuItems, committeeData, tasksData } from './HEMJawatankuasaData';
...
export const HEMJawatankuasa: React.FC = () => {
  ...
  const handleDownloadPdf = () => {
    ...
    const generate = () => {
        ...
        const opt = {
            margin: [10, 10, 10, 10],
            filename: `Jawatankuasa_HEM_${activeLabel.replace(/\s+/g, '_')}.pdf`,
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="text-center md:text-left w-full">
                <div className="flex items-center justify-center md:justify-start gap-2 text-xs font-semibold tracking-wider text-black uppercase mb-1">
                  <span>Hal Ehwal Murid</span>
                  <ChevronRight className="w-3 h-3" />
                  <span>Jawatankuasa</span>
                </div>
                <h1 className="text-2xl font-bold text-black text-center md:text-left">Pengurusan Jawatankuasa HEM</h1>
                <p className="text-sm text-black mt-1 text-center md:text-left">Senarai jawatankuasa dan ahli bagi unit Hal Ehwal Murid.</p>
              </div>
              <button 
