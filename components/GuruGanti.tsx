...
export const GuruGanti: React.FC = () => {
  ...
  const handlePrint = () => {
      // 1. Set Title for Filename (Browsers use page title as default filename)
      ...
      // 3. Reset title (optional, slightly delayed)
      setTimeout(() => {
  ...
  return (
    <div className="font-poppins min-h-screen text-gray-100">
        ...
        <style>{`
            @media print {
                ...
                /* Show the preview modal and its children */
                #print-preview-modal, #print-preview-modal * {
                    visibility: visible;
                    color: black !important; /* Force black text */
                }

                /* Position the print content correctly */
                #print-preview-modal {
                    position: absolute;
                    left: 0;
                    top: 0;
                ...
                /* Hide UI elements inside modal (buttons, etc) */
                .no-print {
        ...
        <div className="bg-transparent p-6 mb-6 border-l-8 border-red-600 print:hidden relative">
            <div className="max-w-7xl mx-auto flex items-center justify-between relative z-10">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-wide font-montserrat text-white">PENGURUSAN GURU GANTI</h1>
                        <h2 className="text-sm font-light tracking-widest text-gray-300 uppercase">Sistem Penjanaan Automatik (SITIGANTI)</h2>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto pb-20 print:p-0 print:max-w-none">
            ...
            <section className="bg-[#1C2541] rounded-xl shadow-lg border border-gray-700 p-6 mb-8 relative overflow-hidden print:hidden">
                ...
                <h3 className="text-xl font-bold mb-6 flex items-center text-white">
                    <span className="bg-[#0B132B] p-2 rounded-lg mr-3 text-[#C9B458] border border-gray-600">
                        <Icons.UserX />
                    </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                    ...
                    <div className="lg:col-span-1">
                        ...
                        <div className="relative">
                            <input 
                                ...
                                className="w-full bg-[#0B132B] border border-gray-600 rounded-lg px-3 py-2 text-white focus:border-[#C9B458] outline-none [&::-webkit-calendar-picker-indicator]:filter-[invert(1)]"
                            />
                        </div>
                        <p className="text-[10px] text-gray-400 mt-1 font-bold bg-white/5 inline-block px-2 py-1 rounded">
                    ...
                    <div className="lg:col-span-1">
                        ...
                        <select 
                            ...
                            className="w-full bg-[#0B132B] border border-gray-600 rounded-lg px-3 py-2 text-white focus:border-[#C9B458] outline-none"
                        >
                    ...
                    <div className="lg:col-span-1 flex items-end">
                        <button 
                            ...
                            className="w-full bg-[#C9B458] hover:bg-yellow-500 text-[#0B132B] font-bold py-2 px-4 rounded-lg transition-colors flex items-center justify-center gap-2 shadow-lg h-[42px]"
                        >
                            <Icons.Plus /> Tambah
                        </button>
                    </div>
                ...
                {absentList.length > 0 && (
                    <div className="overflow-x-auto rounded-lg border border-gray-700">
                        <table className="min-w-full text-left">
                            <thead className="bg-[#0B132B] text-[#C9B458] text-xs uppercase font-bold">
                                <tr>
                                    <th className="px-6 py-3">Nama Guru</th>
                                    <th className="px-6 py-3">Sebab</th>
                                    <th className="px-6 py-3">Status</th>
                                    <th className="px-6 py-3 text-center">Tindakan</th>
                                </tr>
                            </thead>
                            <tbody className="bg-[#1C2541] divide-y divide-gray-700 text-sm">
                                {absentList.map(record => (
                                    <tr key={record.id} className="hover:bg-[#253252]">
                                        <td className="px-6 py-4 font-medium text-white">{record.teacherName}</td>
                                        <td className="px-6 py-4 text-gray-400">{record.reason}</td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-1 inline-flex text-xs leading-5 font-bold rounded-full ${record.status === 'PERLU_DIGANTI' ? 'bg-red-900/50 text-red-400 border border-red-800' : 'bg-green-900/50 text-green-400 border border-green-800'}`}>
                                                {record.status === 'PERLU_DIGANTI' ? 'Perlu Ganti' : 'Tidak Perlu'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <button onClick={() => handleRemoveAbsent(record.id)} className="text-red-400 hover:text-red-300 p-2 hover:bg-red-900/20 rounded-full transition-colors">
                                                <Icons.Trash />
                                            </button>
                                        </td>
                                    </tr>
            ...
            <section className="bg-[#1C2541] rounded-xl shadow-lg border border-gray-700 p-6 mb-8 relative overflow-hidden print:hidden">
                ...
                <h3 className="text-xl font-bold mb-6 flex items-center text-white">
                    <span className="bg-[#0B132B] p-2 rounded-lg mr-3 text-red-500 border border-gray-600">
                        <Icons.ShieldOff />
                    </span>
                    Langkah 2: Pengecualian Ganti
                </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-1 lg:col-span-1">
                        ...
                        <div className="flex gap-2">
                            <select 
                                ...
                                onChange={(e) => setSelectedExempt(e.target.value)}
                                className="w-full bg-[#0B132B] border border-gray-600 rounded-lg px-3 py-2 text-white focus:border-red-500 outline-none text-sm"
                            >
                                <option value="">-- Pilih Guru --</option>
                                {availableForExemption.map(name => (
                                    <option key={name} value={name}>{name}</option>
                                ))}
                            </select>
                            <button 
                                onClick={handleAddExempt}
                                className="bg-red-900/50 hover:bg-red-900 text-red-200 border border-red-800 px-4 rounded-lg font-bold"
                            >
                                +
                            </button>
                        </div>
                        <p className="text-[10px] text-gray-500 mt-2 italic">Guru yang dipilih di sini TIDAK akan disenaraikan sebagai guru ganti.</p>
                    </div>
                    ...
                    <div className="md:col-span-1 lg:col-span-2 bg-[#0B132B] rounded-xl p-4 border border-gray-700 min-h-[100px]">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {exemptedTeachers.length === 0 && <span className="text-gray-600 text-xs italic p-2">Tiada guru dikecualikan.</span>}
                            {exemptedTeachers.map(teacher => (
                                <span key={teacher} className="bg-red-900/30 text-red-300 border border-red-800 rounded-lg px-3 py-1 text-xs flex items-center gap-2">
                                    {teacher}
                                    <button onClick={() => handleRemoveExempt(teacher)} className="hover:text-white font-bold text-red-500 hover:text-red-200">×</button>
                                </span>
                            ))}
                        </div>
                    </div>
            ...
            <section className="bg-[#1C2541] rounded-xl shadow-lg border border-gray-700 p-6 relative overflow-hidden print:hidden">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                    <h3 className="text-xl font-bold flex items-center text-white">
                        <span className="bg-[#0B132B] p-2 rounded-lg mr-3 text-[#3A506B] border border-gray-600">
                            <Icons.Refresh />
                        </span>
                        Langkah 3: Penjanaan Jadual
                    </h3>
                    {absentList.length > 0 && (
                        <button 
                            onClick={generateSchedule}
                            disabled={isProcessing}
                            className={`mt-4 md:mt-0 px-6 py-2 rounded-lg font-bold text-white shadow-md transition-all flex items-center gap-2 ${isProcessing ? 'bg-gray-600 cursor-not-allowed' : 'bg-[#3A506B] hover:bg-[#4a6382] border border-gray-500'}`}
                        >
                            {isProcessing ? 'Sedang Menjana...' : 'Jana Jadual Automatik'}
                        </button>
                    )}
                </div>

                {!isGenerated && !isProcessing && absentList.length > 0 && (
                    <p className="text-gray-500 italic text-center py-8 bg-[#0B132B] rounded-xl border border-dashed border-gray-700">
                        Klik butang 'Jana Jadual Automatik' untuk mendapatkan cadangan guru ganti.
                    </p>
                )}
                    ...
                    <div className="space-y-8 animate-in fade-in">
                        {absentList.filter(a => a.status === 'PERLU_DIGANTI').map((absentTeacher) => {
                            ...
                            return (
                                <div key={absentTeacher.id} className="border border-gray-700 rounded-xl overflow-hidden bg-[#0B132B]">
                                    <div className="bg-[#253252] px-6 py-3 border-b border-gray-700 flex justify-between items-center">
                                        <h4 className="font-bold text-white flex items-center gap-2">
                                            <span className="text-red-400">GURU TIDAK HADIR:</span> {absentTeacher.teacherName}
                                        </h4>
                                        <span className="text-xs text-[#C9B458] font-bold bg-[#0B132B] px-3 py-1 rounded border border-[#C9B458]">{teacherAssignments.length} Kelas</span>
                                    </div>
                                    
                                    <div className="overflow-x-auto">
                                        <table className="min-w-full text-left">
                                            <thead>
                                                <tr className="bg-[#0B132B] text-gray-400 text-xs uppercase font-bold border-b border-gray-700">
                                                    <th className="px-6 py-3">Masa</th>
                                                    <th className="px-6 py-3">Kelas</th>
                                                    <th className="px-6 py-3">Subjek</th>
                                                    <th className="px-6 py-3 w-1/3">Guru Ganti (Cadangan Sistem)</th>
                                                    <th className="px-6 py-3 text-center">Padam</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-gray-700 text-sm">
                                                {teacherAssignments.length === 0 ? (
                                                    <tr>
                                                        <td colSpan={5} className="px-6 py-4 text-center text-gray-500 italic">Tiada kelas perlu diganti.</td>
                                                    </tr>
                                                ) : (
                                                    teacherAssignments.map((assign, idx) => {
                                                        const slotLabel = TIME_SLOTS.find(s => s.id === assign.slotId)?.label || assign.slotId;
                                                        return (
                                                            <tr key={`${assign.recordId}-${assign.slotId}`} className={idx % 2 === 0 ? 'bg-[#1C2541]' : 'bg-[#1C2541]/50'}>
                                                                <td className="px-6 py-3 font-mono text-[#C9B458]">{slotLabel}</td>
                                                                <td className="px-6 py-3 font-bold text-white">{assign.className}</td>
                                                                <td className="px-6 py-3 text-gray-300">{assign.subject}</td>
                                                                <td className="px-6 py-3">
                                                                    <select 
                                                                        className="w-full bg-[#0B132B] border border-gray-600 rounded px-2 py-1 text-white focus:border-[#C9B458] outline-none"
                                                                        value={assign.reliefTeacher || ""}
                                                                        onChange={(e) => handleOverrideRelief(assign.slotId, assign.recordId, e.target.value)}
                                                                    >
                                                                        {!assign.reliefTeacher && <option value="">-- Tiada Guru --</option>}
                                                                        {assign.reliefTeacher && <option value={assign.reliefTeacher}>{assign.reliefTeacher} (Cadangan)</option>}
                                                                        
                                                                        <optgroup label="Pilihan Lain">
                                                                            {assign.candidates.map(c => {
                                                                                if (c.name === assign.reliefTeacher) return null;
                                                                                return (
                                                                                    <option key={c.name} value={c.name}>
                                                                                        {c.name} (Beban: {c.currentLoad})
                                                                                    </option>
                                                                                )
                                                                            })}
                                                                        </optgroup>
                                                                ...
                                                                <td className="px-6 py-3 text-center">
                                                                    <button 
                                                                        onClick={() => handleOverrideRelief(assign.slotId, assign.recordId, "")}
                                                                        title="Kosongkan"
                        ...
                        <div className="flex justify-end pt-4 border-t border-gray-700">
                            <button 
                                ...
                                className="bg-[#C9B458] text-[#0B132B] font-black py-3 px-8 rounded-lg shadow-xl flex items-center gap-2 transform transition-all hover:scale-105 border-2 border-[#0B132B]"
                            >
                                <Icons.Eye /> PRATONTON & CETAK PDF
                            </button>
                        </div>
                    </div>
                )}
                    ...
                    <div className="bg-white text-black w-full max-w-[210mm] min-h-[297mm] shadow-2xl relative mx-auto my-auto print:shadow-none print:w-full print:max-w-none">
                        ...
                        <div className="no-print sticky top-0 left-0 right-0 bg-[#3A506B] text-white p-4 flex justify-between items-center shadow-md z-50 mb-8 rounded-t-lg">
                            ...
                            <div className="flex gap-3">
                                ...
                                <button 
                                    ...
                                    className="bg-teal-600 hover:bg-teal-500 text-white px-6 py-2 rounded-lg font-bold shadow-lg flex items-center gap-2 transition-colors"
                                >
                                    <Icons.Download /> Muat Turun PDF
                                </button>
                                <button 
                                    onClick={handlePrint} 
                                    className="bg-[#C9B458] hover:bg-yellow-500 text-[#0B132B] px-6 py-2 rounded-lg font-bold shadow-lg flex items-center gap-2 transition-colors"
                                >
                                    🖨️ Cetak / Simpan PDF
                                </button>
                            </div>
                        ...
                        <div id="pdf-content" className="p-10 font-serif text-black">
                            ...
                            <div className="flex items-center gap-4 border-b-2 border-black pb-6 mb-8">
                                <img 
                                    src="https://i.postimg.cc/7P9SQBg6/smaam_background_BARU.png" 
                                    className="h-28 w-auto object-contain" 
                                    alt="Logo Sekolah" 
                                />
                                <div className="flex-1 text-center text-black">
                                     <h1 className="text-3xl font-extrabold uppercase tracking-wide mb-1">JADUAL GURU GANTI</h1>
                                     <h2 className="text-xl font-bold uppercase text-black">{getFormattedDate(selectedDate)}</h2>
                                     <p className="text-sm font-semibold mt-1 uppercase text-black tracking-widest">SMA Al-Khairiah Al-Islamiah Mersing</p>
                                </div>
                            </div>
                            ...
                            {absentList.filter(a => a.status === 'PERLU_DIGANTI').map((absentTeacher) => {
                                ...
                                return (
                                    <div key={absentTeacher.id} className="mb-10 break-inside-avoid text-black">
                                        <div className="mb-4 bg-gray-100 p-3 border-l-4 border-black">
                                            <p className="font-bold text-lg uppercase text-black">{absentTeacher.teacherName}</p>
                                            <p className="text-sm italic text-black">Sebab: {absentTeacher.reason}</p>
                                        </div>

                                        <table className="w-full text-left border-collapse border border-black mb-4 text-sm text-black">
                                            <thead>
                                                <tr className="bg-gray-200 text-black">
                                                    <th className="border border-black px-3 py-2 font-bold uppercase w-32 text-center text-black">Masa</th>
                                                    <th className="border border-black px-3 py-2 font-bold uppercase w-48 text-black">Kelas / Subjek</th>
                                                    <th className="border border-black px-3 py-2 font-bold uppercase text-black text-center">Guru Ganti</th>
                                                    <th className="border border-black px-3 py-2 font-bold uppercase w-32 text-center text-black">CATATAN</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {teacherAssignments.map((assign) => {
                                                    ...
                                                    return (
                                                        <tr key={assign.slotId}>
                                                            <td className="border border-black px-3 py-2 font-semibold text-xs text-center align-middle text-black">{slotLabel}</td>
                                                            <td className="border border-black px-3 py-2 align-middle text-black">
                                                                <div className="font-bold text-base text-black">{assign.className}</div>
                                                                <div className="text-xs uppercase bg-black text-white inline-block px-1 rounded mt-1">{assign.subject}</div>
                                                            </td>
                                                            <td className="border border-black px-3 py-2 font-bold uppercase align-middle text-lg text-black text-center">
                                                                {assign.reliefTeacher || "TIADA GURU"}
                                                            </td>
                            ...
                            <div className="mt-20 pt-10 border-t border-gray-400 break-inside-avoid text-black">
                                <div className="flex justify-between items-end px-12">
                                    <div className="text-center w-1/3">
                                        <p className="mb-20 font-bold text-sm uppercase text-black">Disediakan Oleh,</p>
                                        <div className="border-t border-black w-full mt-2"></div>
                                        <p className="font-bold text-xs mt-2 uppercase tracking-wide text-black">Penyelaras Jadual Ganti</p>
                                    </div>
                                    <div className="text-center w-1/3">
                                        <p className="mb-20 font-bold text-sm uppercase text-black">Disahkan Oleh,</p>
                                        <div className="border-t border-black w-full mt-2"></div>
                                        <p className="font-bold text-xs mt-2 uppercase tracking-wide text-black">Pengetua / PK Pentadbiran</p>
                                    </div>
                                </div>
                                <div className="mt-12 text-center text-[10px] text-black pt-4 border-t border-dotted border-gray-400">
                                    Dicetak secara automatik melalui Sistem Pengurusan Digital SMAAM pada {new Date().toLocaleString('ms-MY')}
                                </div>
                            </div>
                        </div>
                    </div>
